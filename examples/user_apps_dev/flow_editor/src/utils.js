...
// Play audio for a specific node
function playAudio(nodeId) {
    const node = flowEditor.nodes.get(nodeId);
    if (!node || !node.audioElement) {
        console.warn('No audio element available for node:', nodeId);
        return;
    }
    
    // Play the audio
    node.audioElement.play().then(() => {
        updateAudioNodeStatus(nodeId, 'Playing...');
        updateAudioControls(nodeId, true);
    }).catch(error => {
        console.error('Failed to play audio:', error);
        updateAudioNodeStatus(nodeId, 'Playback failed');
    });
}

// Stop audio for a specific node
function stopAudio(nodeId) {
    ...
    updateAudioNodeStatus(nodeId, 'Stopped');
    updateAudioControls(nodeId, false);
}

// Update audio node status display
function updateAudioNodeStatus(nodeId, status) {
    const nodeElement = document.getElementById(nodeId);
    if (nodeElement) {
        const statusElement = nodeElement.querySelector('.audio-status');
        if (statusElement) {
...
function updateAudioControls(nodeId, isPlaying) {
    const nodeElement = document.getElementById(nodeId);
    if (nodeElement) {
        const playBtn = nodeElement.querySelector('.audio-play-btn');
        const stopBtn = nodeElement.querySelector('.audio-stop-btn');
